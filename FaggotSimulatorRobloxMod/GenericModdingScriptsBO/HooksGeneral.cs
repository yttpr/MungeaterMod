...
namespace FSRMod
{
    public static class HooksGeneral
    {
        ...
        {
            EZExtensions.PCall(Tim.Menu, "tim in menu");
            orig(self);
        }
        public static void InitializeCombat(Action<CombatManager> orig, CombatManager self)
        {
            EZExtensions.PCall(Tim.Combat, "tim in battle");
            orig(self);
        }
        ...
        public static void PostNotification(Action<CombatManager, string, object, object> orig, CombatManager self, string call, object sender, object args)
        {
            ...
            if (call == TriggerCalls.OnMoved.ToString() && args is IntegerReference hold && sender is IUnit unit && unit.ContainsPassiveAbility(Tim.Passive))
            {
                unit.IncreaseBoneSpurs();
                if (unit.IsUnitCharacter)
                {
                    foreach (CombatSlot slot in CombatManager.Instance._stats.combatSlots.CharacterSlots)
                    {
                        if (slot.SlotID == hold.value)
                        {
                            if (slot.HasUnit) slot.Unit.IncreaseBoneSpurs();
                            break;
                        }
                    }
                }
        ...
        {
            EZExtensions.PCall(Tim.Menu, "Tim in overworld");
            orig(self);
        }
